...
export async function getMessagesForUserId(userInternalId: number, isAdmin: boolean): Promise<QueryResult> {
    return DatabaseService.executeQuery(Constants.QUERIES.NOTIFICATION.GET_MESSAGES_FOR_USER_ID(userInternalId, isAdmin));
}

export async function addNotificationForUserWithId(userInternalId: number, sender: number, message: string, type: string, info: string): Promise<QueryResult> {
    return DatabaseService.executeQuery(Constants.QUERIES.NOTIFICATION.ADD_FOR_USER(userInternalId, sender, message, type, info));
}
